...
async def get_web_app_data(session_name):
    ...
    try:
        async with TelegramClient(session_path, api_id, api_hash) as client:
            try:
                notcoin = await client.get_entity("birdx2_bot")
                msg = await client(functions.messages.RequestWebViewRequest(
                    peer=notcoin,
                    bot=notcoin,
                    platform="android",
                    url="https://birdx.birds.dog/"
                ))
                webappdata_global = msg.url.split('https://birdx.birds.dog/#tgWebAppData=')[1].replace("%3D","=").split('&tgWebAppVersion=')[0].replace("%26","&")
...
async def create_query_id():
    ...
    root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    query_file_path = os.path.join(root_dir, 'query/birdx.txt')

from bot.utils import Colors  # Import kelas Colors dari utils.py
...
class Tomarket:
    ...
    def user_login(self, query):
        ...
        try:
            ...
            return print_timestamp(f"{Colors.RED + Style.BRIGHT}[ {e} ]{Style.RESET_ALL}")
        ...
        try:
            ...
            if data is not None:
                if data['status'] == 0:
                    ...
                    print_timestamp(f"{Colors.GREEN + Style.BRIGHT}[ Game Started Please Wait {int(total_seconds)} Seconds ]{Style.RESET_ALL}")
                    slp = random.randint(30, 35)
                    sleep(slp)
                    self.claim_game(token=token, points=point)
                elif data['status'] == 500:
                    print_timestamp(f"{Colors.YELLOW + Style.BRIGHT}[ No Chance To Play Game ]{Style.RESET_ALL}")
                else:
                    print_timestamp(f"{Colors.RED + Style.BRIGHT}[ {data['message']} ]{Style.RESET_ALL}")
            else:
    ...
    def claim_game(self, token: str, points: int):
        ...
        try:
            ...
            if data is not None:
                if data['status'] == 0:
                    print_timestamp(f"{Colors.GREEN + Style.BRIGHT}[ Game Claimed {data['data']['points']} ]")
                elif data['status'] == 500:
                    print_timestamp(f"{Colors.YELLOW + Style.BRIGHT}[ Game Not Start ]")
            ...
            if data is not None:
                if data['status'] == 0:
                    return print_timestamp(
                            f"{Colors.GREEN + Style.BRIGHT}[ Daily Claim ]{Style.RESET_ALL}"
                            f"{Colors.BLUE + Style.BRIGHT} | {Style.RESET_ALL}"
                            f"{Colors.YELLOW + Style.BRIGHT}[ Day {data['data']['today_game']} ]{Style.RESET_ALL}"
                            f"{Colors.BLUE + Style.BRIGHT} | {Style.RESET_ALL}"
                            f"{Colors.BLUE + Style.BRIGHT}[ Points {data['data']['today_points']} ]{Style.RESET_ALL}"
                    )
                    
                elif data['status'] == 400:
                    return print_timestamp(
                            f"{Colors.YELLOW + Style.BRIGHT}[ Already Check Daily Claim ]{Style.RESET_ALL}"
                            f"{Colors.BLUE + Style.BRIGHT} | {Style.RESET_ALL}"
                            f"{Colors.YELLOW + Style.BRIGHT}[ Day {data['data']['today_game']} ]{Style.RESET_ALL}"
                            f"{Colors.BLUE + Style.BRIGHT} | {Style.RESET_ALL}"
                            f"{Colors.BLUE + Style.BRIGHT}[ Points {data['data']['today_points']} ]{Style.RESET_ALL}"
                    ...
                    return print_timestamp(f"{Colors.RED + Style.BRIGHT}[ {data['message']} ]{Style.RESET_ALL}")
            else:
        ...
        try:
            ...
            while True:
                ...
                if data is not None:
                    ...
                    if data['status'] == 0:
                        print_timestamp(f"{Colors.GREEN + Style.BRIGHT}[ Start Farm ]{Style.RESET_ALL}")
                        if remaining_time.total_seconds() > 0:
                            hours, remainder = divmod(remaining_time.total_seconds(), 3600)
                            minutes, seconds = divmod(remainder, 60)
                            print_timestamp(f"{Colors.YELLOW + Style.BRIGHT}[ Please Wait {int(hours)} Hours {int(minutes)} Minutes {int(seconds)} Seconds To Claim Farm ]{Style.RESET_ALL}")
                            break
                        else:
                            self.claim_farm(token=token)
                            break
                    elif data['status'] == 500:
                        print_timestamp(f"{Colors.YELLOW + Style.BRIGHT}[ Farm Already Started ]{Style.RESET_ALL}")
                        if remaining_time.total_seconds() > 0:
                            hours, remainder = divmod(remaining_time.total_seconds(), 3600)
                            minutes, seconds = divmod(remainder, 60)
                            print_timestamp(f"{Colors.YELLOW + Style.BRIGHT}[ Please Wait {int(hours)} Hours {int(minutes)} Minutes {int(seconds)} Seconds To Claim Farm ]{Style.RESET_ALL}")
    ...
    def claim_farm(self, token: str):
        ...
        try:
            ...
            while True:
                ...
                if data is not None:
                    if data['status'] == 0:
                        print_timestamp(f"{Colors.GREEN + Style.BRIGHT}[ Claimed {data['data']['points']} From Farm ]")
            ...
            for task in default:
                if task['status'] == 0 and task['type'] == "mysterious":
                    ...
                    if date_part == end_time:
                        print_timestamp(f"{Colors.YELLOW + Style.BRIGHT}[ Claiming {task['title']} ]{Style.RESET_ALL}")
    ...
    def clear_task(self, query, token, task):
        if task['status'] == 1:
            print_timestamp(f"{Colors.YELLOW + Style.BRIGHT}[ You Haven't Finish Or Start This {task['title']} Task ]{Style.RESET_ALL}")
            trys = 9
            while True:
                if trys <= 0:
                    break
            ...
            print_timestamp(f"{Colors.YELLOW + Style.BRIGHT}[ Starting {task['title']} ]{Style.RESET_ALL}")
    ...
    def claim_tasks(self, token: str, task_id: int):
        ...
        data = response.json()
        if data['status'] == 0:
            print_timestamp(f"{Colors.GREEN + Style.BRIGHT}[ Claimed ]{Style.RESET_ALL}")
        elif data['status'] == 500:
            print_timestamp(f"{Colors.YELLOW + Style.BRIGHT}[ You Haven't Finish Or Start This Task ]{Style.RESET_ALL}")
        elif data['status'] == 401:
            print_timestamp(f"{Colors.RED + Style.BRIGHT}[ Invalid Task ]")
        else:
            print_timestamp(f"{Colors.RED + Style.BRIGHT}[ {data['message']} ]{Style.RESET_ALL}")

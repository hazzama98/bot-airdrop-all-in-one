...
async def get_web_app_data(session_name):
    ...
    try:
        async with TelegramClient(session_path, api_id, api_hash) as client:
            try:
                notcoin = await client.get_entity("Tomarket_ai_bot")
                ...
                webappdata_global = msg.url.split('https://mini-app.tomarket.ai/#tgWebAppData=')[1].replace("%3D","=").split('&tgWebAppVersion=')[0].replace("%26","&")
...
async def create_query_id():
    ...
    root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    query_file_path = os.path.join(root_dir, 'query/tomarket.txt')
